 ...
 */
public class ModelPanelController1 implements IModelPanelController {
    ...
    public String getApiManager(String method, String tag) {
        ...
        if (method.equals(GET)) {
            ...
            sb.append("\n{return apiManager.").append(ctag).append("(map,BNHelper.getHeader(map,BaseApplication.token,\"/resources/xsa/lookup/searchLookupValues\",Function.ACCOUNT_M.getName()));}");
        } else if (method.equals(POST)) {
            ctag = "update" + tag;
            sb.append(" Observable<" + tag + "Json> ").append("" + ctag).append("(RequestBean bean)");
            sb.append("\n{return apiManager.").append(ctag).append("(bean, BNHelper.postHeader(bean, \"/resources/eih/accounts\",  BaseApplication.token, Function.ACCOUNT_I.getName()));}");
        } else {
            ctag = "create" + tag;
            sb.append(" Observable<" + tag + "Json> ").append("" + ctag).append("(RequestBean bean)");
            sb.append("\n{return apiManager.").append(ctag).append("(bean, BNHelper.postHeader(bean, \"/resources/eih/accounts\",  BaseApplication.token, Function.ACCOUNT_I.getName()));}");
    ...
    public String getModel(String method, String tag) {
        ...
        if (method.equals(GET)) {
            ...
            sb.append("Subscription sub = ApiManager.").append(ctag).append("(map).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).subscribe(new Action1<" + cjson + ">() {");
            sb.append("\n@Override\n" +
            ...
            ctag = "update" + tag;
            sb.append("" + ctag).append("(RequestBean bean);");
            sb.append("\n").append(modelImpHeader);
            sb.append("\n").append("private CURLinstener<" + cjson + "," + cjson + "," + cjson + "> mListener;\n\n");
            sb.append("Subscription sub = ApiManager.").append(ctag).append("(bean).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).subscribe(new Action1<"+cjson+">() {");
            sb.append(" \n@Override\n" +
    ...
    public String getPresenterCode(String method, String tag) {
        ...
        String presenteroi = " private "+ tag +" Model mModel;\n\n mModel = new "+tag+"ModelImp("+tag+"Listener);\n\n";
        String listener = "private CURLinstener<" + cj + "> "+tag+"Listener = new CURLinstener<" + cj + ">() {\n" +
                "        @Override\n" +
                "        public void onRetrieveSuccess(" + cjson + " json) {\n" +
                "            mView.hideProgress();\n" +
                "            Logger.d(getClass().getSimpleName() + \" -> retrive"+ tag + " : %s\",json.toString());"+
                "            //mView.retrive"+tag+"(json);\n" +
                "        }\n" +
                "\n" +
                "        @Override\n" +
                "        public void onUpdateSuccess(" + cjson + " json) {\n" +
                "            mView.hideProgress();\n" +
                "            //mView.update"+tag+"(json);\n" +
                "            Logger.d(getClass().getSimpleName() + \" -> update"+ tag + " : %s\",json.toString());"+
                "        }\n" +
                "\n" +
                "        @Override\n" +
                "        public void onCreateSuccess(" + cjson + " json) {\n" +
                "            mView.hideProgress();\n" +
                "            Logger.d(getClass().getSimpleName() + \" -> create"+ tag + " : %s\",json.toString());"+
                "            //mView.create"+tag+"(json);\n" +
                "        }\n" +
                "\n" +
                "        @Override\n" +
                "        public void onError(Throwable e) {\n" +
                "            mView.hideProgress();\n" +
                "            Logger.e(getClass().getSimpleName() + \" -> "+ tag + "出现异常 : %s\",e.toString());"+
                "        }\n" +
                "    };";
